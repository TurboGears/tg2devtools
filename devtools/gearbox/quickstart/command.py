...
class QuickstartAPICommand(Command):
    ...
    def get_description(self):
        return 'Creates a new TurboGears2 API project'

    def get_parser(self, prog_name):
        parser = super(QuickstartAPICommand, self).get_parser(prog_name)
    ...
    def take_action(self, opts):
        ...
        quickstart_path = os.path.abspath(os.path.dirname(__file__))

        # Workaround for templates ported from Paste
        # which check for 'True' instead of True
        template_vars = dict(vars(opts))

        QuickstartAPITemplate().run(os.path.join(quickstart_path, 'template-api'),
                                 opts.name, template_vars)
